...
class GoodsreceiptController extends Controller
{
    ...
    public function update(Request $request)
    {
        ...
        foreach ($filteredItems as $row) {
            ...
            if ($row['final_qty'] != 0) {
                ...
                if (!empty($product)) {
                    ...
                    // $inventoryData['qty'] = optional($inventoryExist)->qty + $row['final_qty'];
        ...
        $log = ['module' => 'Goods Receipt', 'action' => 'Goods Receipt Created', 'description' => 'Goods Receipt Created'];
        captureActivity($log);

        if ($modal->getChanges()) {
            $new_changes = $modal->getChanges();
            $log = ['module' => 'Goods Receipt', 'action' => 'Goods Receipt Updated', 'description' => 'Goods Receipt Updated For Item ' . $modal->item_code];
