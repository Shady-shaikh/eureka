...
class BankingpaymentController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        if ($request->ajax()) {
            $bankingpayment = Bankingpayment::where(['fy_year' => session('fy_year'), 'company_id' => session('company_id')])->with('get_partyname')->orderby('created_at', 'desc')->get();
            ...
            return DataTables::of($bankingpayment)
                ...
                ->addColumn('action', function ($bankingpayment) {
                    ...
                    if (request()->user()->can('View Banking Payments')) {
                        // dd("yes");
                        $actionBtn .= '<a href="' . route('admin.bankingpayment.view', ['id' => $bankingpayment->banking_payment_id]) . '
    ...
    public function create()
    {
        ...
        return view('backend.bankingpayment.create', compact('party', 'party_code', 'banks'));
    }

    public function show($id)
    {

        $GLOBALS['breadcrumb'] = [['name' => 'Banking Payment', 'route' => "admin.bankingpayment"], ['name' => 'View', 'route' => ""]];
        $roles = Role::pluck('name', 'id')->all();
        $purchaseorder = Bankingpayment::where('banking_payment_id', $id)->first();
        ...
        return view('backend.bankingpayment.show', compact('roles', 'bank_details', 'bill_booking_items', 'purchaseorder', 'party', 'invoice'));
    }
