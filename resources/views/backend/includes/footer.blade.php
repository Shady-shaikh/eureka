...
<script>
    function validateInputZero(input, currentIndex) {
        var qtyArray = @json(array_values($qtyArray ?? []));
...
{{-- to restrcit user to enter only letters in item desc filed --}}
<script>
...
<script>
    $(document).ready(function() {
        ...
        $('#curr_year').change(function() {
            ...
            $.ajax({
                type: 'GET',
                url: '{{ route('admin.change_fy') }}', // Use your actual route name
                data: {
                    year: selectedYear
                },
