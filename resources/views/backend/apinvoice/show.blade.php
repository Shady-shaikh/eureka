...
@section('title', 'View A/P Invoice')
...
<div class="col-sm-12 text-sm-end mb-1">
  ...
  @if($goodsservicereceipts->split_purchaseorder == 1)
  @include('backend.apinvoice.invoice_format_split', ['roles'=>$roles,'invoice'=>$goodsservicereceipts,'party'=>$party,'bill_address'=>$bill_address,'ship_address'=>$ship_address,'contact'=>$contact,'download'=>false])
  @else
  @include('backend.apinvoice.invoice_format', ['roles'=>$roles,'invoice'=>$goodsservicereceipts,'party'=>$party,'bill_address'=>$bill_address,'ship_address'=>$ship_address,'contact'=>$contact,'download'=>false])
  @endif
      <div class="row">
        <div class="col-lg-12 text-center">
  <a href="javascript:void(0);" class="btn btn-primary" onclick="PrintElem('.printable');">Print</a>
          <a href="{{ route('admin.apinvoice') }}" class="btn btn-secondary">Back</a>
        </div>
      </div>
...
<script>
    ...
    <script type="text/javascript">
        ...
        {
          //alert(data);
            var mywindow = window.open('', '{{ $goodsservicereceipts->bill_no }}', 'height=600,width=800');
            mywindow.document.write('<html><head><title>{{ $goodsservicereceipts->bill_no }}</title>');
